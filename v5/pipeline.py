...
def refresh_state(
    ...
    stages = [
        ...
        # Generation stages (character_sheets, storyboards, scripted_clips,
        # extended_sequences, final_assembly) are NOT hardcoded here. Per
        # docs/19_EXECUTION_STATUS.md, pipeline_state.json holds intake stages
        # only; generation status is derived on read by execution_status.py
        # from the authority order (user review > plans/graph manifest >
        # runner job log > named output files > live queue overlay).
    ]
