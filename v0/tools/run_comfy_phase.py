...
def phase_items(manifest, phase):
    ...
    groups = {
        ...
        "motion_proofs": [item for item in manifest["videos"] if item["duration_seconds"] == 5],
        "extended_clips": [item for item in manifest["videos"] if item["duration_seconds"] == 10],
    }
    if phase == "all":
...
def stage_storyboard(item, files, input_root):
    ...
    target = input_root / "scene_factory_generated" / parts[-4] / parts[-3] / parts[-2] / f"{parts[-1]}.png"
...
def main():
    ...
    for index, item in enumerate(items, 1):
        ...
        if item.get("execution_phase") in {"motion_proofs", "extended_clips"}:
