...
> V1 is the active reformed pipeline. The complete pre-reformation state is preserved in the sibling `v0` snapshot. See [V1_REFORMATION.md](V1_REFORMATION.md) for the subject-binding, prop, anatomy, and review-gate changes.
...
For ongoing feature-length episodic production, asset/script swaps, and manual ComfyUI workflow editing, use the [Feature-Episode Production Handbook](docs/17_FEATURE_EPISODE_PRODUCTION_HANDBOOK.md). New projects include a series episode worksheet and reusable JSON review forms.
